...
function SOM(options) {
    ...
    for (var i = 0; i < _self.symbols.length; i++) {
        ...
        //var csv1 = loader("public/data/"+_self.symbols[i]+".csv");
        var csv1 = JSON.parse(fs.readFileSync("public/data/" + _self.symbols[i] + ".json"));
    ...
    for (var i = 0; i < _self.symbols.length; i++) {
        if (minLength > _self.csv[_self.symbols[i]].length) {
            minLength = _self.csv[_self.symbols[i]].length;
        }
    }
